...
class SeoPage implements SeoPageInterface
{
    ...
    public function addMeta($type, $name, /* string */ $content, array $extras = [])
    {
        if (!is_string($content)) {
            @trigger_error(sprintf('Passing meta content of type %s in %s is deprecated since version 2.7.1 and will be unsupported in version 3. Please cast the value to a string first.', gettype($content), __METHOD__), E_USER_DEPRECATED);
        }
